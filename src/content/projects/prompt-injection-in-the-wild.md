...
- **Fake system-metadata framing.** The injected text is dressed up as "Edge browser tabs metadata," complete with a fabricated `edge_all_open_tabs` JSON structure whose `pageTitle`/`pageUrl` fields are wrapped in fabricated `WebsiteContent_...` tags — mimicking the shape of a legitimate content-sanitization boundary rather than looking like freeform text. That wrapper plausibly serves double duty: an attempt to make the payload read as pre-sanitized to a model, and a tracking token for the phishing kit itself.
