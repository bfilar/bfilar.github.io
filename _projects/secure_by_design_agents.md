...
description: How we architected ASA and ADÉ for adversarial production — Sublime Security, 2026
img:
importance: 1
...
- **ADÉ (Autonomous Detection Engineer)** generates new detections in MQL (Sublime's domain-specific language), runs backtests, and surfaces results with precision scores and reasoning summaries. (Evaluation methodology: [CAMLIS 2025]({{ '/projects/camlis_2025/' | relative_url }}).)
...
Moving to higher autonomy is something organizations do *deliberately, based on demonstrated evidence* — not something that happens automatically. The gating logic is the one I formalized in [*Trust, Then Autonomy*]({{ '/projects/trust_then_autonomy/' | relative_url }}): autonomy is granted by the customer, not declared by the vendor.
...
- The deployment framework that determines *when* the autonomy gates are opened: [Trust, Then Autonomy]({{ '/projects/trust_then_autonomy/' | relative_url }})
- The evaluation methodology behind ADÉ's quality gating: [CAMLIS 2025]({{ '/projects/camlis_2025/' | relative_url }})
